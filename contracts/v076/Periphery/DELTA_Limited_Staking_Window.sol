...
contract DELTA_Limited_Staking_Window {
  ...
  function endLiquidityDeployment() public {
    ...
    uint256 deltaForPool = deltaForPoolAndReserve.mul(percentOfBalanceToReserves).div(1000);

    // transfer to pool
    deltaToken.transfer(deltaWethUniswapPair, deltaForPool);
    ...
    require(deltaToken.balanceOf(deltaWethUniswapPair) == deltaForPool, "LSW did not get permissions to send directly to balance");
    ...
    uint256 deltaForReserve = deltaToken.balanceOf(address(this));
    deltaToken.transfer(reserveVaultAddress, deltaForReserve); // queried again in case of rounding problems
    ...
    IRESERVE_VAULT(reserveVaultAddress).setRatio(
        deltaForPool.div(MAX_ETH_POOL_SEED)
    );
